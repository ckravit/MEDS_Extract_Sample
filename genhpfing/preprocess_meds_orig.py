...
def find_boundary_between(tuples_list, start, end):
    ...
    if start_index >= end_index:  
        ...
        print(f"DEBUG: tuples_list={tuples_list}")  
        print(f"DEBUG: starts={starts}, ends={ends}")  
...
def meds_to_remed(
    ...
    def analyze_problematic_span(text_span):  
        print(f"Span analysis for: '{text_span}'")  
        for i, char in enumerate(text_span):  
            print(f"  Position {i}: '{char}' (Unicode: {ord(char)}, ASCII: {char.isascii()})")  
        
        # Check for any remaining problematic characters  
        problematic_chars = ['*', '+', '?', '(', ')', '[', ']', '^', '$', '"', "'", '<', '>', '@', '#', '%', '&', '=']  
        found_chars = [char for char in text_span if char in problematic_chars]  
        if found_chars:  
            print(f"  Found problematic characters: {found_chars}") 
    ...
    def meds_to_remed_unit(row):
        ...
        # Add debug information before the problematic loop  
        print(f"DEBUG: Processing {len(events)} events")  
        for idx, event in enumerate(events):  
            print(f"DEBUG: Event {idx}: '{event[:100]}...' (length: {len(event)})")  

        # print(f"Pre-tokenization text: '{events[i]}'")  
        # print(f"Digit separation applied: '{events[i]}' -> check for ' . ' patterns")  
        ...
        # After tokenization  
        # print(f"Token offsets: {tokenized_events[i].offsets}")  
        # print(f"Tokens: {tokenized_events[i].input_ids}")

        # Add debug for tokenization results  
        print(f"DEBUG: Tokenized {len(events)} events")  
        for i, event in enumerate(events):  
            print(f"DEBUG: Event {i} tokens: {tokenized_events['input_ids'][i][:20]}...")  
            print(f"DEBUG: Event {i} offsets: {tokenized_events[i].offsets[:10]}...")  
        
        lengths_before_padding = tokenized_events["attention_mask"].sum(axis=1)
        print(f"Event token lengths: {lengths_before_padding}")  
        print(f"Max token length in batch: {lengths_before_padding.max()}")   
        ...
        for i, digit_offset in enumerate(digit_offsets): 
            print(f"DEBUG: Processing event {i} with {len(digit_offset)} digit spans")  
            print(f"DEBUG: Event {i} text: '{events[i]}'")  
            print(f"DEBUG: Event {i} digit_offsets: {digit_offset}")  
         
            for j, (start, end) in enumerate(digit_offset):  
                print(f"DEBUG: Processing digit span {j}: start={start}, end={end}")  
                print(f"DEBUG: Text span: '{events[i][start:end]}'")  
                print(f"DEBUG: Available token offsets: {tokenized_events[i].offsets[:lengths_before_padding[i]-1]}")  

                # THIS IS WEHRE THE BELOW WAS INDENTED 
                try: 
                    ...
                    print(f"DEBUG: Successfully mapped to tokens {start_index}-{end_index}")  
                except AssertionError:  
                    # print(f"DEBUG: ASSERTION FAILED for digit span {j} in event {i}")  
                    # print(f"DEBUG: Failed span text: '{events[i][start:end]}'")  
                    # print(f"DEBUG: Full event text: '{events[i]}'")  
                    # print(f"DEBUG: Original row data:") 
                    # for col_name in ["code", "numeric_value"] + rest_of_columns:  
                    #     if "id" not in col_name and col_name in column_name_idcs:  
                    #         print(f"DEBUG:   {col_name}: {row[column_name_idcs[col_name]][i] if i < len(row[column_name_idcs[col_name]]) else 'N/A'}")  
                    # raise
                    print(f"ASSERTION ERROR DEBUG:")  
                    print(f"Event {i} full text: '{events[i]}'")  
                    print(f"Problematic span [{start}:{end}]: '{events[i][start:end]}'")   
                    print(f"Context around span: '{events[i][max(0,start-10):end+10]}'")   
                    print(f"All digit offsets for this event: {digit_offset}")   
                    print(f"Current digit span index: {digit_offset.index((start, end))}")  
                    
                    # Check for specific problematic patterns  
                    span_text = events[i][start:end]  
                    analyze_problematic_span(span_text) 
                    if '.' in span_text:  
                        print(f"Contains period: '{span_text}'")  
                    if any(char in span_text for char in ['*', '+', '?', '(', ')', '[', ']', '^', '$']):  
                        print(f"Contains regex metacharacter: '{span_text}'")  
        ...
        for i, col_name_offset in enumerate(col_name_offsets):
            ...
            for start, end in col_name_offset:
                try:  
                    ...
                    print(f"Error processing event {i} in row {row}")  
                    print(f"Event text: {events[i]}")  
                    print(f"Digit offsets: {digit_offsets[i]}")  
                    # print("- Asserion Error")
                    raise
